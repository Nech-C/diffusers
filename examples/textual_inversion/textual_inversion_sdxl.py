...
#
# Modified to add:
#  - --load_unet_in_8bit: load UNet in 8-bit via bitsandbytes
#  - --offload_to_cpu / --offload_folder / --offload_state_dict: HF device_map auto offloading
#
# Usage examples:
#  accelerate launch textual_inversion.py \
#    --pretrained_model_name_or_path="Ine007/waiNSFWIllustrious_v140" \
#    --train_data_dir="./soukaku/images" \
#    --placeholder_token="<char: soukaku>" \
...
def parse_args():
    ...
    # -----------------------------------------------------------------------
    # New: quantization and offloading options
    parser.add_argument(
        "--load_unet_in_8bit",
        action="store_true",
        help="Load UNet in 8-bit using bitsandbytes (requires bitsandbytes and transformers support).",
    )
    parser.add_argument(
        "--offload_to_cpu",
        action="store_true",
        help="Automatically offload large model weights to CPU using device_map='auto' and offload_folder.",
    )
    parser.add_argument(
        "--offload_folder",
        type=str,
        default="./hf_offload",
        help="Folder used for CPU offloading (device_map='auto' offload folder).",
    )
    parser.add_argument(
        "--offload_state_dict",
        action="store_true",
        help="If set, pass offload_state_dict=True to from_pretrained (helpful for very large models).",
    )
    # -----------------------------------------------------------------------
...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        # list files in dir (only files) - ignore directories
        self.image_paths = [
            os.path.join(self.data_root, file_path)
            for file_path in os.listdir(self.data_root)
            if os.path.isfile(os.path.join(self.data_root, file_path))
        ]
...
def main():
    ...
    # Prepare output dir and hub repo if requested
    ...
    # Build kwargs for from_pretrained calls
    common_from_pretrained_kwargs = {"revision": args.revision, "variant": args.variant}

    if args.offload_to_cpu:
        os.makedirs(args.offload_folder, exist_ok=True)
        # add offload kwargs (these are supported in recent transformers/accelerate)
        common_from_pretrained_kwargs.update(
            {
                "device_map": "auto",
    ...
    # UNet kwargs: support 8-bit quantization via BitsAndBytesConfig if requested
    unet_from_pretrained_kwargs = dict(common_from_pretrained_kwargs)
    if args.load_unet_in_8bit:
        try:
            from transformers import BitsAndBytesConfig
        except Exception as e:
            raise ImportError(
                "Could not import BitsAndBytesConfig from transformers. "
                "Make sure you have a recent `transformers` and have installed `bitsandbytes`."
            ) from e

        try:
            import bitsandbytes  # noqa: F401
        except Exception as e:
            raise ImportError("bitsandbytes is required for --load_unet_in_8bit. Install with `pip install bitsandbytes`.") from e

        bnb_config = BitsAndBytesConfig(load_in_8bit=True)
        unet_from_pretrained_kwargs["quantization_config"] = bnb_config

        # For best memory reduction, use device_map auto when loading 8-bit UNet.
        if "device_map" not in unet_from_pretrained_kwargs:
            unet_from_pretrained_kwargs["device_map"] = "auto"

    # Load VAE and UNet (may be device-mapped/quantized)
    vae = AutoencoderKL.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", **common_from_pretrained_kwargs
    )

    unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", **unet_from_pretrained_kwargs
    )

    # Add the placeholder token(s)
    ...
    # Prepare with accelerator (do NOT pass device-mapped models like unet/vae to prepare)
    ...
    # but skip manual .to() for device-mapped models (hf_device_map/device_map)
    def is_device_mapped(model):
        # HF dispatch commonly stores hf_device_map or device_map on the object
        try:
            if getattr(model, "hf_device_map", None):
                return True
            # some loaded models expose .device_map attr
            if getattr(model, "device_map", None):
                return True
        except Exception:
            return False
        return False

    def maybe_move_model(model, model_name: str):
        if is_device_mapped(model):
            logger.info(f"Skipping manual .to() for {model_name} because it's device-mapped/offloaded.")
            return
        model.to(accelerator.device, dtype=weight_dtype)
        logger.info(f"Moved {model_name} to {accelerator.device} with dtype={weight_dtype}.")

    maybe_move_model(unet, "UNet")
    maybe_move_model(vae, "VAE")
    maybe_move_model(text_encoder_2, "text_encoder_2")
    ...
    if args.max_train_steps is None:
        ...
        if num_training_steps_for_scheduler != args.max_train_steps * accelerator.num_processes:
            logger.warning(
                f"The length of the 'train_dataloader' after 'accelerator.prepare' ({len(train_dataloader)}) does not match expected."
            )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate([text_encoder_1, text_encoder_2]):
                ...
                added_cond_kwargs = {"text_embeds": text_embeds_for_added, "time_ids": add_time_ids}
                ...
                # ensure we don't update other embeddings
                ...
                with torch.no_grad():
                    ...
                    accelerator.unwrap_model(text_encoder_2).get_input_embeddings().weight[index_no_updates_2] = orig_embeds_params_2[index_no_updates_2]
